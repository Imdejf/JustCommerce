...
{
    public static class UpdateProductTypeProperty
    {
        ...
        public sealed class Handler : IRequestHandlerWrapper<Command, Unit>
        {
            ...
            {
                var existProductType = await _unitOfWorkAdministration.ProductType.GetWithProductTypePropertyByIdAsync(request.ProductTypeId, cancellationToken);
                ...
                foreach(var productTypeProperty in existProductType.ProductTypeProperty)
                {
                    _unitOfWorkAdministration.ProductTypeProperty.Remove(productTypeProperty);
                }
